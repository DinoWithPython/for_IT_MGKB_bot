...
def menu_func(button, user_id):
    buttons_menu = (
        button(text=easy_work["consultation"][0], callback_data="consultation"),
        button(text=easy_work["miac"][0], callback_data="miac"),
        button(text=easy_work["egisz"][0], callback_data="egisz"),
        button(text="Установка ПО", callback_data="install_po"),
        button(text=easy_work["frv_frk"][0], callback_data="frv_frk"),
        button(text=easy_work["emias_pol"][0], callback_data="emias_pol"),
        button(text=easy_work["emias_stas"][0], callback_data="emias_stas"),
        button(text="Прочие активности", callback_data="any_activity"),
        button(text="Как я там натрудился?🏥", callback_data="my_activity"),
        button(
            text="Покажи мои прочие активности🏥", callback_data="my_activity_another"
        ),
        button(text="Вернуться в обычный режим🌏", callback_data="back_normal_mode"),
    )
    if user_id == config.USERS[2]:
        return buttons_menu + (
            button(text="Отчет за текущий месяц 🔥", callback_data="report_now_month"),
            button(
                text="Отчет за предыдущий месяц 🔥", callback_data="report_last_month"
            ),
        )
    return buttons_menu
...
def activity_mode_func(bot):
    def create_record_db(message):
        ...
        bot.send_message(
            message.chat.id, "Прочая активность учтена.", reply_markup=kb_activity_res
        )

    @bot.message_handler(func=lambda message: message.text == "Режим учёта активности")
    ...
    def use_callback_data(callback):
        ...
        if callback.data in easy_work:
            ...
            db.adding_any_work(user_id, easy_work[callback.data][0], config.USERS)
            ...
            for key, value in menu_install.items():
                kb_install_po.add(button(text=value[0], callback_data=key))
            bot.edit_message_text(
                chat_id=call.chat.id,
                message_id=call.id,
                text="Что ставили?",
            ...
            user_id = callback.from_user.id
            db.adding_any_work(user_id, menu_install[callback.data][0], config.USERS)
            ...
            send = bot.send_message(
                call.chat.id, "Отправьте сообщение о проделеннай работе."
            )
            # Создаем поток
            ...
            bot.send_message(call.chat.id, "С чем работаем?", reply_markup=kb_1)
            ...
            if not succesess_work_count:
                bot.send_message(
                    call.chat.id, "Странно, записей об обычных работах нет."
                )
            else:
                bot.send_message(call.chat.id, text_work_count)
            time.sleep(0.5)
            if not succesess_work:
                bot.send_message(call.chat.id, "Записей об индивидуальных работах нет.")
            else:
                bot.send_message(call.chat.id, text_work)
            time.sleep(0.5)
            bot.send_message(call.chat.id, "Отчет предоставлен выше.", reply_markup=kb5)
            ...
            if not succesess_work:
                ...
                bot.send_message(call.chat.id, text_work)
            time.sleep(0.5)
            bot.send_message(call.chat.id, "Отчет предоставлен выше.", reply_markup=kb6)
